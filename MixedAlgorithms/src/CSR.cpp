...
unsigned int CSRMatrix::getRows() const {
    return m;
}

double CSRMatrix::getValue(unsigned int i, unsigned int j) const {
    if (i >= m || j >= n) {
...
const std::vector<unsigned int>& CSRMatrix::getRowPtr() const {
    return row_ptr;
}

const std::vector<unsigned int>& CSRMatrix::getColIndices() const {
    return col_indices;
}
